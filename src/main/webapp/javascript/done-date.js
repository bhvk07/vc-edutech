...
document.getElementById ("add_installment").addEventListener ("click", add , false);
...
$(".form_date").datetimepicker("setDate", new Date()); //sets current date 
...
//        $("#add_installment").on("click",function(){
    ...
    var r = document.getElementById('i-details');
     ...
     var rest_rows;
    var arr = [];
     $("#i-details tr:nth-child(n + 2)").each(function(){
         rest_rows = $(this).find('input[name^="amt_installment"]').attr('id').toString();
         arr.push(rest_rows);
     });
     ...
     for(j=1; j<=no; j++){
         ...
         alert(dt);
         document.getElementsByClassName("display-date")[j-1].setAttribute("value",dt.toLocaleDateString());
         var brk = document.getElementsByClassName("d-row")[j-1].setAttribute("value",res);
    ...
    /* function fetch(var x)
    {
       ...
       $(function () {
           $("#installment_table").on('click', '#remove-row1', function() {
