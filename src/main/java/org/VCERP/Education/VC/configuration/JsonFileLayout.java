...
@Plugin(name = "JsonFileLayout", category = Node.CATEGORY, elementType = Layout.ELEMENT_TYPE, printObject = true)
public final class JsonFileLayout extends AbstractStringLayout {
	...
	protected JsonFileLayout(final Configuration config, final boolean locationInfo, final boolean properties,
			final boolean complete, final boolean eventEol, final String headerPattern, final String footerPattern,
			final Charset charset, final KeyValuePair[] additionalFields) {
	...
	private static LogEvent convertMutableToLog4jEvent(final LogEvent event) {
		// MutableLogEvent but don't know how...
	...
	private final String format(final LogEvent event) {
		...
		/*
		 * json(sb, "layout.start", layoutStartTime); json(sb,
		 * "layout.sequence", layoutSequence.incrementAndGet());
		 */
		// Basic Info
		// Date logTime = new Date(event.getTimeMillis());
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
		json(sb, "timestamp", sdf.format(event.getTimeMillis()));
		json(sb, "threadName", event.getThreadName());
		json(sb, "threadId", event.getThreadId());
		json(sb, "level", event.getLevel().toString());
		json(sb, "loggerName", event.getLoggerName());
		json(sb, "lineNumber", event.getSource().getLineNumber());
		/* json(sb, "ContextMap", event.getContextData().toMap());
		Map<String, String> contextMap = event.getContextData().toMap();
		Set<String> contextData = event.getContextData().toMap().keySet();
		for (String key : contextData) {
	...
	@PluginFactory
	public static JsonFileLayout createLayout( //
		...
		return new JsonFileLayout(config, locationInfo, properties, complete, eventEol, footerPattern, headerPattern,
				charset, additionalFields);
	}
}
